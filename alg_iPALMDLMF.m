function y3=alg_iPALMDLMF(Y,Sd,St,cv_setting,nr_fold,left_out,use_WKNKN,K,eta,use_W_matrix)
...
%  y3:  prediction matrix
    ...
    % get best parameters
    [k,lambda_l,lambda_d,lambda_t,lambda_s,p,num_iter] = alg_iPALMDLMF_parest(Y,Sd,St,cv_setting,nr_fold,left_out,use_WKNKN,K,eta,use_W_matrix);
    fprintf('k%g\t\t%g\t%g\t%g\t\t',k,lambda_l,lambda_d,lambda_t,lambda_s);
    ...
    if ~use_W_matrix
        [A,B] = alg_iPALMDLMF_predict(Y,A,B,Ld,Lt,lambda_l,lambda_d,lambda_t,lambda_s,num_iter);
    ...
    % else if weight matrix W is being used (i.e. classifier='wgrmf')
        ...
        W = ones(size(Y));
        W(test_ind) = 0;
        [A,B] = alg_iPALMDLMF_predict(Y,A,B,Ld,Lt,lambda_l,lambda_d,lambda_t,lambda_s,num_iter,W);
    end

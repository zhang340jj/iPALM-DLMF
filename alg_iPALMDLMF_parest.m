function [k,lambda_l,lambda_d,lambda_t,lambda_s,p,num_iter]=alg_iPALMDLMF_parest(Y,Sd,St,cv_setting,nr_fold,left_out,use_WKNKN,K,eta,use_W_matrix)
    ...
    range_lambda_s  = [   0  10^-4  10^-3  10^-2  10^-1];
                ...
                for lambda_t=range_lambda_dt
                    ...
                    for i=1:nr_fold
                        ...
                        if ~use_W_matrix    % grmf                                                          %
                            [A,B] = alg_iPALMDLMF_predict(y2,A,B,Ld,Lt,lambda_l,lambda_d,lambda_t,lambda_s,num_iter);     %
                        else                % wgrmf                                                         % PREDICT
                            [A,B] = alg_iPALMDLMF_predict(y2,A,B,Ld,Lt,lambda_l,lambda_d,lambda_t,lambda_s,num_iter,W);   %
                        end                                                                                 %
                        y3 = A*B';                                                                          %
